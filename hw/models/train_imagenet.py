...
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet18',
                    ...
                    help='model architecture: ' +
                        ' | '.join(model_names) +
                        ' (default: resnet18)')
parser.add_argument('-z', '--zfp-mode', metavar='MODE', default=None, 
                    choices=['lossless', 'precision', 'accuracy', 'rate'],
                    help='zfp mode: ' +
                        ' | '.join(['lossless', 'precision', 'accuracy', 'rate']) +
                        ' (default: lossless)')
# Add a boolean flag -r
parser.add_argument('-r', '--reversed', action='store_true', help='reverse the order of the variables')
...
def main():
    ...
    if args.multiprocessing_distributed:
        ...
        for zfp_mode in zfp_modes:
            ...
            variables = tolerances if args.zfp_mode == 'accuracy' else rates
            if args.zfp_mode == 'lossless':
                variables = [0]
            
            for variable in variables:
                print(f"\nRunning {args.zfp_mode=}, {variable=}")
                result = main_worker(variable, args.gpu, ngpus_per_node, args)
                print(result)
...
def main_worker(variable, gpu, ngpus_per_node, args):
    global best_acc1
    global g_total_codec_sec
    global g_total_transfer_sec
    global g_total_steps
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        diverged = train(variable, train_loader, model, criterion, optimizer, epoch, device, args)
        end_time = perf_counter()
        
        total_duration_sec += end_time - start_time
        g_total_steps += len(train_loader)
        ...
        acc1_max = max(acc1, best_acc1)
        if acc1_max == best_acc1:
            unchanged_counter += 1
        else:
            unchanged_counter = 0
        best_acc1 = acc1_max
        ...
        result = {
            'arch': args.arch,
            'zfp_mode': args.zfp_mode,
            'batch_size': args.batch_size,
            'epoch': epoch,
            'variable': variable, 
            'accuracy': best_acc1.item(), 
    ...
    print(f'Training Done ({diverged=})')
    return result

def train(variable, train_loader, model, criterion, optimizer, epoch, device, args):
    global g_total_codec_sec
    global g_total_transfer_sec
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        #* Conduct ZFP compression given the mode
...
class ProgressMeter(object):
    ...
    def display(self, batch):
        entries = [f"{g_arch}({g_mode}) |"] + [self.prefix + self.batch_fmtstr.format(batch)]

...
int main(int argc, char** argv)
{
    ...
    std::cout << "Initialized double gradient input of size " << in_fp_gradients.size() << std::endl;

    //* Initialize output.
    std::vector<int, aligned_allocator<int>> out_zfp_gradients(GRAD_BLOCK_SIZE);
    std::cout << "Initialized int gradient output of size " << out_zfp_gradients.size() << std::endl;
    ...
    std::cout << "Created a kernel which uses the program object previously loaded\n";
    ...
    std::cout << "Duration (including memcpy out): " << duration << " seconds" << std::endl;
    std::cout << "Overall grad values per second = " << GRAD_BLOCK_SIZE / duration << std::endl;
    ...
    for(int i = 0; i < GRAD_BLOCK_SIZE; i++) {
        if (out_zfp_gradients[i] != sw_out_zfp_gradients[i]) {
            std::cout << "ERROR at [" << i << "] HW: " << out_zfp_gradients[i] << " != SW: " << sw_out_zfp_gradients[i] << std::endl;
            match = false;
            break;
        }
    }

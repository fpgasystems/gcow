...
/**
 * @brief Gather full 4x4x4x4 (4^D) double values from a serialized 4D matrix.
...
/**
 * @brief Gather partial nx*ny*nz*nw double values from a serialized 4D matrix
...
void gather_2d_block(double *block, const double *raw, ptrdiff_t sx,
                     ptrdiff_t sy);
...
void encode_strided_2d_block(uint64 *encoded, const double *raw, ptrdiff_t sx,
                             ptrdiff_t sy);
